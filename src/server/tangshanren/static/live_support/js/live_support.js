...
function getMessages() {
    ...
    $('.chat').each(function(index, item) {      
        ...
        var last_message_control = $(item).find('.msg-item');
        ...
        if (last_message_control.length > 0) {
            ...
            var last_message_id = $(item).find('.msg-item').attr('id').replace('message_', '');
...
function sendMessage(event) {
    var message = $(event.target).parent().parent().find('.message_box').val();
    
    if (message.trim() != '') {
        var chat_id = $(event.target).parent().parent().find('.chat_id').val();
        var url = $(event.target).parent().parents('form').attr('action');
        var last_message_id = "0";
        if ( $('.chat').find('.msg-item:last').length > 0 ){
            last_message_id = $('.chat').find('.msg-item:last').attr('id').replace('message_', '');
        }
        var args = {
            'message': message,
...
function scrollAll() {
    $('.chat').each(function(index, control) {
