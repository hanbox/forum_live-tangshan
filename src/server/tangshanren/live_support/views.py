...
from live_support.models import Chat, ChatMessage, SupportGroup, Friendship
...
def client_get_messages(request, chat_uuid):
    ...
    for message in messages:
        ...
        info = user_mine.objects.filter(username=message.name)
        nickname = info[0].nickname
        message_list.append({
            'name': escape(nickname),
            'message': escape(message.message),
            'pk': message.pk,
        })
    chats = {
...
def client_chat(request, chat_uuid, talkto):
    chat = get_object_or_404(Chat, hash_key=chat_uuid)
    message_form = ChatMessageForm(request.POST or None)
    if message_form.is_valid():
        message = message_form.save(commit=False)
    ...
    return render_to_response('live_support/chat_dedail.html', params,
                              context_instance=RequestContext(request))

def friendslist(request):
    # user = request.user
    # if not request.user.is_authenticated():
    #     return redirect("/user/login/?next=/support/", reverse('spirit:user:auth:login'))

    user_id = request.user.id
    friends = Friendship.objects.filter(userid=user_id)

    return render_to_response('live_support/chat_friends_mobile.html', {"friends" : friends})
...
def deletefriend(request):
    if request.method == "POST":
        ...
        Friendship.objects.filter(roomid=room_id).delete()
