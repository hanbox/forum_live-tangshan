...
from live_support.models import Friendship
...
def _activity(request, pk, slug, queryset, template, reverse_to, context_name, per_page):
    ...
    info = user_mine.objects.filter(username=p_user.username)
...
def topics(request, pk, slug):
    ...
    return _activity(
        ...
        template='spirit/user/profile_topics_mobile.html',
...
def comments(request, pk, slug):
    ...
    return _activity(
        ...
        template='spirit/user/profile_comments_mobile.html',
...
def likes(request, pk, slug):
    ...
    return _activity(
        request, pk, slug,
        queryset=user_comments,
        template='spirit/user/profile_likes_mobile.html',
...
def addfriend(request):
    ...
    if request.method == "POST":
        ...
        chat = Chat()
        chatname = "chatname" + str(userid) + "_" + str(friendid)
        details = chatname
        ...
        friend1.friendsid = userid
        friend1.friendname = username
        friend1.roomid = roomid
        friend1.save()

        return HttpResponsePermanentRedirect("/support/")
